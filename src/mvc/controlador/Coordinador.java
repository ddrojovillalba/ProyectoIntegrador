...
public class Coordinador implements ActionListener, KeyListener, ItemListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource()==miVentanaLogin.getBtnSalir()) {
			...
			int resp=JOptionPane.showConfirmDialog(null, "¿Quiere salir de la aplicación","WARNING", JOptionPane.YES_NO_OPTION);
			if (resp==0) {
				System.exit(0);
			}
		...
		if (e.getSource()==miVentanaAplicacion.getSideBar().getBtnAcercaDe()){
			...
			System.out.println("Click en editar proyectos ");
			JOptionPane.showMessageDialog(miVentanaAplicacion, "Proyecto realizado por el grupo Codies \n \n © 2023 - Universidad Europea");
		}
		...
		if (e.getSource()==miVentanaAplicacion.getPestañaAreas().getBtnConsulta()) {
			...
			//-------============================================ Eventos Ventana buscar Alumnos ====================================---------------------------------------
			
		
				// Boton BUSCAR
		
							// Generamos la tabla con los usuarios que cumplen el criterio de busqueda, comprobándolo en los campos nombre o expediente
		...
		if (e.getSource()==miBuscarAlumnos.getBtnBuscar()) {
			...
			miBuscarAlumnos.generarTabla(resultadoBusqueda);
				...
				// Botón DETALLE
							...
							// Muestra el detlle del usuario seleccionado en la tabla
		...
		if (e.getSource()==miBuscarAlumnos.getBtnDetalle()) {
				...
				int userID=Integer.parseInt((String) miBuscarAlumnos.getTablaAlunmos().getModel().getValueAt(filaSeleccionada, 0));
		...
		if (e.getSource()==miDetalle.getBtnEditar()) {
			...
			if (miDetalle.getBtnEditar().getText().equals("EDITAR")) {   
				...
				miDetalle.cambiarVistaAEdicion();
						...
						// Si estamos en modo edición (pondrá CANCELAR y en botón ACEPTAR pondrá GUARDAR), cambiamos otra vez a Detalle	
				
				
			}else if ((miDetalle.getBtnEditar().getText().equals("CANCELAR")) && (miDetalle.getBtnAceptar().getText().equals("GUARDAR"))) {  
				
				miDetalle.cambiarVistaDetalle();
				
						// Si estamos en modo registro (pondrá CANCELAR y en botón ACEPTAR pondrá REGISTRAR), cerramos la ventana y limpiamos campos	
				
				
			}else if ((miDetalle.getBtnEditar().getText().equals("CANCELAR")) && (miDetalle.getBtnAceptar().getText().equals("REGISTRAR"))) {
		...
		if (e.getSource()==miDetalle.getBtnBorrar()) {
				...
				int resp= JOptionPane.showOptionDialog(miVentanaAplicacion, "¿Quieres borrar el registro?", null, 0, 0, null, null, e);
				System.out.println(resp);
		  ...
		  // -------============================================== Eventos Ventana Buscar Proyectos ============================---------------------
		
		
						// Botón BUSCAR
		
									// Generamos la tabla con los proyectos que cumplen el criterio de busqueda, comprobándolo en los campos nombre o grupo, año o curso
		...
		if (e.getSource()==miBuscarProyectos.getBtnVerDetalle()) {
			  ...
			  //muestra el detalle del proyecto seleccionado en la tabla
				...
				if (filaSeleccionada>=0){
				int prID= Integer.parseInt((String)miBuscarProyectos.getTblDatosProyectos().getModel().getValueAt(filaSeleccionada, 0));
				miDetallesProyectos.setVisible(true);
				miDetallesProyectos.rellenarTabla(miUsuarioDTO.cargarAlumnos());
				miDetallesProyectos.mostrarProyecto(miProyectoDTO.mostrarDetalle(prID));
				miDetallesProyectos.cargarIntegrantes(miIntegrantesDTO.CargarIntegrantes(prID));
		...
		if (e.getSource()==miDetallesProyectos.getBtnCerrar()) {
			...
			if (miDetallesProyectos.getBtnCerrar().getText().equals("CERRAR")) {
				...
				miDetallesProyectos.limpiarCampos();
							...
							// Si pone actualizar, comprobamos que los campos obligatorios están rellenos y actualizamos el registro del proyecto mostrado en BD
				...
				if (camposRellenos) {
					...
					JOptionPane.showMessageDialog(null, "Debes rellenar los campos marcados con asterisco");
							...
							// Si pone GUARDAR, comprobamos que los campos obligatorios están rellenos y insertamos el registro del proyecto en BD
		...
		if (e.getSource()==miDetallesProyectos.getBtnEditar()) {
			...
			else if ((miDetallesProyectos.getBtnEditar().getText().equals("CANCELAR")) && (miDetallesProyectos.getBtnCerrar().getText().equals("ACTUALIZAR"))){
				miDetallesProyectos.cambiarVistaDetalle();
				
							// Si pone CANCELAR y en BOTON CERRAR pone ACTUALIZAR ocultamos la vista
				
			}else if ((miDetallesProyectos.getBtnEditar().getText().equals("CANCELAR")) && (miDetallesProyectos.getBtnCerrar().getText().equals("GUARDAR"))) {
				miDetallesProyectos.setVisible(false);
			}
	...
	private void comprobarRespuesta(UsuarioVO usuarioLogeando, String resp) {
		if (resp.equals("Logged")) {
			...
			JOptionPane.showMessageDialog(null, "Debes rellenar ambos campos", "Campos sin rellenar", JOptionPane.WARNING_MESSAGE);
