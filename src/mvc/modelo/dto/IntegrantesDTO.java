...
public class IntegrantesDTO {
	...
	public ArrayList<UsuarioVO> CargarIntegrantes(int prID) {
		...
		String consulta;
		consulta="SELECT users.user_id, name, expediente FROM users INNER JOIN integrantes ON (users.user_id = integrantes.user_id) WHERE integrantes.proyecto_id= ?";  // Devuelveme todos los usuarios
	...
	/*public String borrarIntegrantes(int idPR) {
		...
		ConexionDB conexion = new ConexionDB();
		String consulta="DELETE FROM integrantes WHERE proyecto_id = ?";
		connection=conexion.establecerConexion();
	...
	public String añadirIntegrantes(int idPR, int[] integrantes) {
		...
		ConexionDB conexion = new ConexionDB();
		String consulta="INSERT INTO integrantes (proyecto_id, user_id) VALUES (?, ?)";
		connection=conexion.establecerConexion();
		...
		try {
			...
			for (int id_usuario : integrantes) {
				prepStatement.setInt(1, idPR);
				prepStatement.setInt(2, id_usuario);
				prepStatement.execute();
			}
			resultado="OK";
